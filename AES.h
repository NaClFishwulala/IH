...
int keyExpansion(const uint8_t* key, uint32_t keyLen, AesKey* aesKey) {
    ...
    if (NULL == key || NULL == aesKey) {
        printk(KERN_INFO "keyExpansion param is NULL\n");
        return -1;
    }

    if (keyLen != 16) {
        printk(KERN_INFO "keyExpansion keyLen = %d, Not support.\n", keyLen);
        return -1;
    }
...
int aesEncrypt(const uint8_t* key, uint32_t keyLen, const uint8_t* pt, uint8_t* ct, uint32_t len) {
    ...
    if (NULL == key || NULL == pt || NULL == ct) {
        printk(KERN_INFO "param err.\n");
        return -1;
    }

    if (keyLen > 16) {
        printk(KERN_INFO "keyLen must be 16.\n");
        return -1;
    }

    if (len % BLOCKSIZE) {
        printk(KERN_INFO "inLen is invalid.\n");
        return -1;
    }
